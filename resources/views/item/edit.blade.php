                    ...
                    <div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
                        <div class="sm:col-span-2">
                            ...
                            <input type="text" name="title" id="name"
                                ...
                                placeholder="Type Note Title" value="{{ $notes->title }}">
                        </div>
                        <div class="sm:col-span-2">
                            <label for="created_at"
                                class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">
                                Created At</label>
                            <input type="date" value="{{ $notes->created_at->format('Y-m-d') }}"  name="created_at" id="date"
                                class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-primary-600 focus:border-primary-600 block w-full p-2.5 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-primary-500 dark:focus:border-primary-500">
                        </div>
                        <div class="lg:col-span-2">
                            ...
                            <textarea id="description" rows="8" name="description"
                                ...
                                placeholder="Your description here">{{ $notes->description }}</textarea>
                        </div>
                    </div>
                    <button type="submit"
...
$(document).ready(function() {
    $('#noteForm').validate({
        ...
        submitHandler: function(form) {
            ...
            $.ajax({
                url: "{{ route('item.modify',['id'=>$notes->id]) }}",
                type: "POST",
                data: formData,
                processData: false,
                contentType: false,
